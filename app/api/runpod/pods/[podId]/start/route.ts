...
// POST /api/runpod/pods/[podId]/start - Start a pod
export async function POST(request: NextRequest, { params }: { params: Promise<{ podId: string }> }) {
  try {
    const { podId } = await params;

    const data = await runpodFetch(`/pods/${podId}/start`, {
      method: 'POST',
    });
    ...
    return NextResponse.json(
      { error: 'Failed to start pod', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}